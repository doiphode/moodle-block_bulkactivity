...
{
    global $DB;
    $sql = "select id,name from {course_categories} where visible =1 && parent =$parentid";
            ...
            $courselist .= ' <div class="panel panel-default " >
						...
						<h3 class="panel-title categoryname" style="font-weight: 100">
							<a data-toggle="collapse"    aria-expanded="true" aria-controls="collapse_' . $category.'_'.$chidld->id.'"  href="#collapse_' . $category.'_'.$chidld->id.'">
								<i class="indicator fa fa-caret-right" aria-hidden="true" style="color: silver;"></i> ' . $chidld->name . '
							</a>
						</h3>
					</div>
                        <div id="collapse_' . $category.'_'.$chidld->id.'" class="collapse"  data-parent="#accordion_'.$category.'" style="padding-left: 15px;padding-right: 15px;">
